...
export default function RecouvrementFournisseursPage() {
  ...
  const fetchReglements = async () => {
    setIsLoading(true);
    try {
      const response = await fetch("/api/recouvrement/reglements-fournisseurs");
      ...
      toast({
        title: "Erreur",
        description: "Impossible de charger les règlements fournisseurs",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handlePayer = async (id: string, detailIndex?: number, isDetail?: boolean, parentId?: string) => {
    ...
    try {
      ...
      toast({
        title: "Succès",
        description: result.message || "Paiement effectué avec succès",
      });
      ...
      toast({
        title: "Erreur",
        description: error instanceof Error ? error.message : "Impossible d'effectuer le paiement",
        variant: "destructive",
      });
    } finally {
            ...
            mixteDetails.forEach((detail, idx) => {
              if (detail && detail.montant > 0) {
                details.push({
                  ...
                  statut: detail.statut || (detail.type === 'ESPECE' ? 'PAYE' : 'EN_ATTENTE'),
  ...
  // En attente uniquement (exclure espèces déjà payées)
  ...
  const totalPaye = allDetails.filter(d => d.statut === 'PAYE').reduce((sum, d) => sum + d.montant, 0);
  ...
  const tauxPaiement = totalGeneral > 0
    ? Math.round((totalPaye / totalGeneral) * 100)
    : 0;
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {item.fournisseur?.nom} {item.banque && `- ${item.banque}`}
          </p>
        </div>
      </div>
      <div className="flex flex-wrap items-center gap-4 w-full sm:w-auto">
        ...
        {item.statut === 'EN_ATTENTE' && (
          <Button
            ...
            disabled={paiementInProgress === (item.isDetail ? `${item.parentId}_${item.detailIndex}` : item.id)}
            className="whitespace-nowrap"
          >
            {paiementInProgress === (item.isDetail ? `${item.parentId}_${item.detailIndex}` : item.id) ? (
              <>
    ...
    return (
      <div className="flex min-h-screen bg-background flex-col md:flex-row">
        ...
        <div className={cn("flex-1 transition-all duration-300", sidebarClasses)}>
          <Header
            title="Recouvrement Fournisseurs"
            subtitle="Suivi des dettes et paiements fournisseurs"
          />
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-red-600">{formatCurrency(totalGeneral)}</div>
              ...
              <Card className="border-green-500/50">
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-green-600">{formatCurrency(totalPaye)}</div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="pt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-red-600">-{formatCurrency(totalEspeces)}</span>
                  </div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="pt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-red-600">-{formatCurrency(totalCheques)}</span>
                  </div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="pt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-red-600">-{formatCurrency(totalTraites)}</span>
                  </div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="pt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-red-600">-{formatCurrency(totalVirements)}</span>
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <Tabs defaultValue="en-attente">
              <TabsList className="flex flex-wrap">
                <TabsTrigger value="en-attente" className="flex items-center gap-2">
                  <Clock className="h-4 w-4" />
                  À Payer ({enAttenteDetails.length})
                </TabsTrigger>
              ...
              <TabsContent value="historique" className="mt-4">
                <Card>
                  ...
                  <CardContent>
                    {allDetails.filter(d => d.statut === 'PAYE').length === 0 ? (
                      <p className="text-center text-muted-foreground py-8">
